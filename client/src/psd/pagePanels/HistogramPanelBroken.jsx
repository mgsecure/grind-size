...
export default function HistogramPanel({domEl}) {
    ...
    const activeItems = useMemo(() => fullQueue.filter(item => activeIdList.includes(item.id)), [fullQueue, activeIdList])
    ...
    const openSettings = useCallback((id) => {
        setSettingsItem(fullQueue.find(item => item.filename === id)) // TODO fix?
    ...
    const showAggregate = aggregateQueueItem && activeIdList.length === 1

    const legendItems = fullQueue.map((item, idx) => ({id: item.sampleName, color: chartColors[idx]}))
    ...
    const {chartData, lineData, xLabel, yLabel, keys} = useMemo(() => {
        ...
        const itemsWithHist = activeItems && activeItems
            .filter(item => {
                const hist = binSpacing === 'log' ? item.result?.histograms?.log : item.result?.histograms?.linear
        ...
        const firstHist = binSpacing === 'log' ? itemsWithHist[0].result?.histograms.log : itemsWithHist[0].result?.histograms.linear
        ...
        !showAggregate && aggregateQueueItem && lData.push({
            id: 'Aggregate',
            data: cData.map(d => ({
                x: d.bin,
                y: d.Aggregate,
    ...
    }, [xAxis, yAxis, activeItems, binSpacing, aggregateQueueItem, showAggregate])
    ...
    const aggregateBarLineLayer = ({bars, xScale, yScale}) => {
        const lineGenerator = line()
            ...
            .curve(chartCurve === 'linear' ? curveLinear : curveBasis)
        const uniqueBins = []
        ...
        return (aggregateQueueItem && !showAggregate
                ? <path
    ...
    }),[aggregateColor, chartColors, formatXTick, maxY, swapColors, theme.palette.divider, tickLegendColor, xLabel, yLabel])
        ...
        <Paper sx={{p: isDesktop ? 2 : 1, width: '100%'}}>
            ...
            <Stack direction='row' alignItems='center' flexWrap='wrap' justifyContent='space-between'
                ...
                <Stack direction='row' alignItems='center' justifyContent={isDesktop ? 'flex-end' : 'flex-start'}
                    ...
                    <ToggleButtonGroup
                        size='small' value={binSpacing} exclusive onChange={(_, v) => v && setBinSpacing(v)}
                        style={{marginRight: isDesktop ? 15 : 10}}>
                        <ToggleButton value='log' style={{padding: 7.5}}>
                            <ScaleLogIcon width={19} height={19}/>
                        </ToggleButton>
                        <ToggleButton value='linear' style={{padding: 7.5}}>
                            <ScaleLinearIcon width={19} height={19}/>
                        </ToggleButton>
                    </ToggleButtonGroup>

                    <ToggleButtonGroup
                        size='small' value={chartMode} exclusive onChange={(_, v) => v && setChartMode(v)}
                        style={{marginRight: isDesktop ? 15 : 10}}>
                        <ToggleButton value='bar' size='small' style={{padding: 5}}>
                            <BarChartIcon size='small' width={18} height={18}/></ToggleButton>
                        <ToggleButton value='line' size='small' style={{padding: 5}}>
                            <ShowChartIcon size='small' width={18} height={18}/></ToggleButton>
                    </ToggleButtonGroup>

                    <ToggleButtonGroup
                        size='small' value={chartCurve} exclusive onChange={(_, v) => v && setChartCurve(v)}>
                        <ToggleButton value='linear' style={{padding: 7}}>
                            <CurveLinearIcon width={20} height={20}/>
                        </ToggleButton>
                        <ToggleButton value='curve' style={{padding: 7}}>
                            <CurveCardinalIcon width={20} height={20}/>
                        </ToggleButton>
                    </ToggleButtonGroup>
            ...
            <Stack direction='row' flexWrap='wrap' spacing={2} justifyContent='center' sx={{mb: 1, pr: 2, pl: 4}}>
                {legendItems.map(li => (
                    <Box key={li.id} sx={{display: 'flex', alignItems: 'center', gap: 1}} style={{marginTop: 12}}>
